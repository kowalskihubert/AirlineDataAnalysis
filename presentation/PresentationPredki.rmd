---
title: "Mó wi li mi że nikim nie bęęęęędęęęęęę"
...
Opóźnienia kaskadowe, bez względu na pogodę
<details>
...
Opóźniania kaskadowe, gdzie wystąpiło opóźnienie pogodowe
<details>
<summary>Query</summary>
```{sql eval=FALSE, connection=db, }
SELECT
...
from main m
inner join airports a ON (Trim(m.Origin) = Trim('"' from a.iata))
where LateAircraftDelay > WeatherDelay and WeatherDelay > 0 and year between 2000 and 2008
group by city
having totalFlights > 500
order by avgLateAircraftDelay desc
limit 50;
```
</details>
